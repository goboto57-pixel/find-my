...
function isValueEqual(context, a, b) {
	...
	switch (true) {
		...
		case a.type === AST_NODE_TYPES.ThisExpression && b.type === AST_NODE_TYPES.ThisExpression:
			...
			return Traverse.findParent(a, Check.isOneOf(thisBlockTypes)) === Traverse.findParent(b, Check.isOneOf(thisBlockTypes));
		default: {
...
function resolveImportSource(name, initialScope, seen = /* @__PURE__ */ new Set()) {
	...
	if (node.type === AST_NODE_TYPES.VariableDeclarator && node.init != null) {
		...
		if (init.type === AST_NODE_TYPES.MemberExpression && init.object.type === AST_NODE_TYPES.Identifier) return resolveImportSource(init.object.name, initialScope, seen);
		if (init.type === AST_NODE_TYPES.Identifier) return resolveImportSource(init.name, initialScope, seen);
		const arg0 = getRequireExpressionArguments(init)?.[0];
		if (arg0 == null || !isMatching({
...
function resolveObjectType(context, node) {
	...
	switch (node.type) {
		...
		case AST_NODE_TYPES.MemberExpression: return resolveObjectType(context, node.object);
		case AST_NODE_TYPES.AssignmentExpression:
		case AST_NODE_TYPES.AssignmentPattern: return resolveObjectType(context, node.right);
		case AST_NODE_TYPES.LogicalExpression: return resolveObjectType(context, node.left) ?? resolveObjectType(context, node.right);
		...
		case AST_NODE_TYPES.CallExpression: {
			...
			switch (true) {
				case Check.isIdentifier(callee, "Boolean"): return null;
				case Check.isIdentifier(callee, "String"): return null;
				case Check.isIdentifier(callee, "Number"): return null;
				case Check.isIdentifier(callee, "Object"): return {
					kind: "plain",
					node
				};
				case Check.isIdentifier(callee, "Array"): return {
					kind: "array",
					node
				};
				case Check.isIdentifier(callee, "RegExp"): return {
					kind: "regexp",
					node
				};
			}
