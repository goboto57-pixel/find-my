...
function create$13(context) {
	...
	return { CallExpression(node) {
		if (Extract.getCalleeName(node) === "findDOMNode") context.report({
			messageId: "default",
			node
		});
	} };
...
function create$12(context) {
	...
	return { CallExpression(node) {
		if (Extract.getCalleeName(node) === "flushSync") context.report({
			messageId: "default",
			node
		});
	} };
		...
		CallExpression(node) {
			...
			switch (true) {
				case callee.type === AST_NODE_TYPES.Identifier && hydrateNames.has(callee.name):
					...
					return;
				case callee.type === AST_NODE_TYPES.MemberExpression && callee.object.type === AST_NODE_TYPES.Identifier && Extract.getCalleeName(node) === "hydrate" && reactDomNames.has(callee.object.name):
					context.report({
						fix: buildFix$2(context, node),
						messageId: "default",
						node
		...
		CallExpression(node) {
			...
			switch (true) {
				case callee.type === AST_NODE_TYPES.Identifier && renderNames.has(callee.name) && isReturnValueUsed(node):
					...
					return;
				case callee.type === AST_NODE_TYPES.MemberExpression && callee.object.type === AST_NODE_TYPES.Identifier && Extract.getCalleeName(node) === "render" && reactDomNames.has(callee.object.name) && isReturnValueUsed(node):
					context.report({
						messageId: "default",
						node
					});
		...
		CallExpression(node) {
			...
			switch (true) {
				case callee.type === AST_NODE_TYPES.Identifier && renderNames.has(callee.name):
					...
					return;
				case callee.type === AST_NODE_TYPES.MemberExpression && callee.object.type === AST_NODE_TYPES.Identifier && Extract.getCalleeName(node) === "render" && reactDomNames.has(callee.object.name):
					context.report({
						fix: buildFix$1(context, node),
						messageId: "default",
						node
		...
		CallExpression(node) {
			...
			switch (true) {
				case callee.type === AST_NODE_TYPES.Identifier && useFormStateNames.has(callee.name):
					...
					return;
				case callee.type === AST_NODE_TYPES.MemberExpression && callee.object.type === AST_NODE_TYPES.Identifier && Extract.getCalleeName(node) === "useFormState" && reactDomNames.has(callee.object.name):
					context.report({
						fix: buildFix(context, node),
						messageId: "default",
						node
